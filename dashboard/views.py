...
from dashboard.models import RiwayatDeteksi
...
def riwayat_deteksi(request):
    ...
    riwayat = RiwayatDeteksi.objects.select_related(
        'petani', 'lahan', 'hasil_deteksi', 'hasil_deteksi__jenis_hama', 'hasil_deteksi__citra'
    ).all().order_by('-created_at')
    
    # Hitung statistik sederhana
    total_deteksi = riwayat.count()
    tanaman_sakit = riwayat.filter(hasil_deteksi__tingkat_serangan__in=['ringan','sedang','berat']).count()
    perlu_penanganan = riwayat.filter(status_penanganan__in=['belum','proses']).count()

    context = {
        'riwayat': riwayat,
        'total_deteksi': total_deteksi,
        'tanaman_sakit': tanaman_sakit,
...
# ==============================
# Rekomendasi_detail view
# ==============================
def recommendation_view(request):
    detection = Detection.objects.last()

    recommendation = None
    if detection:
        recommendation = Recommendation.objects.filter(
            pest_name__iexact=detection.pest_name
        ).first()
...
def export_pdf(request):
    riwayat_list = RiwayatDeteksi.objects.select_related(
        'hasil_deteksi__jenis_hama',
        'lahan'
    ).order_by('-created_at')

    template = get_template('dashboard/riwayat_pdf.html')
    ...
    response['Content-Disposition'] = (
        f'attachment; filename="riwayat_deteksi_{tanggal}.pdf"'
    )
